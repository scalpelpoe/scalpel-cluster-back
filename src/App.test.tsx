...
describe('App', () => {
  it('renders the wheel, both boxes, and the results placeholder initially', () => {
    ...
    expect(screen.getByText('Desired Notable 2')).toBeTruthy()
  ...
  it('limits the second box to compatible partners', () => {
    ...
    render(<App ctx={ctx} />)
    pick('Desired Notable 1', 'prodig', 'Prodigious Defence')
    fireEvent.change(box('Desired Notable 2').getByPlaceholderText('Search notables'), { target: { value: 'sadist' } })
    expect(box('Desired Notable 2').queryByText('Sadist')).toBeNull()
  })
